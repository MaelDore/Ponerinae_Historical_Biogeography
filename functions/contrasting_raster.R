...
contrasting_raster <- function(x, zmin, zmax)
{
  ...
  continental_mask <- (raster::calc(x, fun = sum) >= 0) - 1
  ...
  community_mask_PA <- (raster::calc(x, fun = sum) > 0) - 1
  ...
  x <- raster::mask(x, mask = community_mask)  # Cut out values that are outside species range
