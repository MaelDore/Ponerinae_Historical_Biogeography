function (x, y, plot = FALSE, force.rooted = FALSE, use.edge.length = FALSE, 
          commons = TRUE, location = "bottomleft", ...) 
{
  ...
  if (rooted1 && rooted2 || force.rooted) {
    ...
    if (plot) {
      ...
      plot(x, use.edge.length = use.edge.length, main = tree1, 
           ...)
      nodelabels(node = which(tmp) + n1, pch = 19, col = "blue", 
                 cex = 2)
      legend(location, legend = paste("Clade absent in", 
                                      tree2), pch = 19, col = "blue")
    }
    ...
    if (plot) {
      plot(y, use.edge.length = use.edge.length, main = tree2, 
           ...)
      nodelabels(node = which(tmp) + n2, pch = 19, col = "red", 
                 cex = 2)
      legend(location, legend = paste("Clade absent in", 
                                      tree1), pch = 19, col = "red")
    }
    ...
    if (ultra1 && ultra2) {
      ...
      BT <- data.frame(paste0(bt1[nodes1], " (", nodes1 + 
                                n1, ")"), paste0(bt2[nodes2], " (", nodes2 + 
                                                   n2, ")"))
  ...
  if (sameTips) {
    ...
    if (plot) {
      ...
      if (ncs) {
        ...
        if (commons) {
          k1 <- which(!is.na(match(pp1, pp2)) & lengths(pp1) > 
                        1)
          k2 <- which(!is.na(match(pp2, pp1)) & lengths(pp2) > 
                        1)
        }
        else {
          k1 <- which(is.na(match(pp1, pp2)) & lengths(pp1) > 
      ...
      text4leg <- if (commons) 
        "Split present in both trees"
      else "Split specific to each tree"
      plot(TR[[1]], "u", use.edge.length = use.edge.length, 
           edge.color = edgecol1, edge.width = edgew1, main = tree1, 
           ...)
      legend(location, legend = text4leg, lty = 1, col = "black", 
             lwd = 5, xpd = NA)
