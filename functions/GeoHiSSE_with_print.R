...
### Modified version of GeoHiSSE function by Jeremy M. Beaulieu from the hisse R package version 2.1.11

# Changes: 
  # Can print progress during simulated annealing and subplex routine if needed
  # Does not work for unbounded subplex routine (bounded.search = FALSE)
...
GeoHiSSE_with_print <- function(phy, data, f=c(1,1,1), turnover=c(1,2,3), eps=c(1,2), hidden.states=FALSE, trans.rate=NULL, assume.cladogenetic=TRUE, condition.on.survival=TRUE,
                                root.type="madfitz", root.p=NULL, sann=TRUE, sann.its=1000, bounded.search=TRUE, max.tol=.Machine$double.eps^.50, mag.san.start=0.5, starting.vals=NULL,
                                turnover.upper=1000, eps.upper=3, trans.upper=100, restart.obj=NULL, ode.eps=0, dt.threads=1,
                                print_level_subplex = 0, # To print progress during the subplex
                                verbose_genSA = FALSE)
{
  ...
  { # Without simulated annealing
    ...
    if (bounded.search == TRUE)
    { 
      # For bounded subplex routine
      cat("Finished. Beginning bounded subplex routine...", "\n")
      opts <- list("algorithm" = "NLOPT_LN_SBPLX", "maxeval" = 100000, "ftol_rel" = max.tol, print_level = print_level_subplex)
  ...
  } else { # With simulated annealing
    
    cat("Finished. Beginning simulated annealing...", "\n")
    out.sann = GenSA(ip, fn=DevOptimizeGeoHiSSEfast, lower=lower, upper=upper, control=list(max.call=sann.its, verbose = verbose_genSA), pars=pars, dat.tab=dat.tab, gen=gen, hidden.states=hidden.states, assume.cladogenetic=assume.cladogenetic, nb.tip=nb.tip, nb.node=nb.node, condition.on.survival=condition.on.survival, root.type=root.type, root.p=root.p, np=np, ode.eps=ode.eps)
...
DevOptimizeGeoHiSSEfast <- function(p, pars, dat.tab, gen, hidden.states, assume.cladogenetic=assume.cladogenetic, nb.tip=nb.tip, nb.node=nb.node, condition.on.survival, root.type, root.p, np, ode.eps) {
  ...
  cache = ParametersToPassGeoHiSSEfast(model.vec=model.vec, hidden.states=hidden.states, assume.cladogenetic=assume.cladogenetic, nb.tip=nb.tip, nb.node=nb.node, bad.likelihood=exp(-300), ode.eps=ode.eps)
...
OrganizeDataGeo <- function(data, phy, f, hidden.states){
  ...
  dat.tab <- data.table::as.data.table(tmp.df)
  data.table::setkey(dat.tab, DesNode)
  for (j in 1:(dim(compD)[2])){
    dat.tab[data.table::data.table(c(1:nb.tip)), paste("compD", j, sep="_") := compD[,j]]
    dat.tab[data.table::data.table(c(1:nb.tip)), paste("compE", j, sep="_") := compE[,j]]
  }
  return(dat.tab)
}
...
SingleChildProbGeo <- function(cache, pars, compD, compE, start.time, end.time, x){
  ...
  if(any(!is.finite(c(compD, compE)))) { # something went awry at a previous step. Bail!
    prob.subtree.cal <- rep(0, ifelse(cache$hidden.states == TRUE, 60, 6))
...
FocalNodeProbGeo <- function(cache, pars, lambdas, dat.tab, generations){
  ...
  data.table::setkey(dat.tab, FocalNode)
  CurrentGenData <- dat.tab[data.table::data.table(generations)]
  if(cache$hidden.states == TRUE){
    ...
    data.table::setkey(dat.tab, DesNode)
    #gens <- data.table::data.table(c(generations))
    rows <- dat.tab[.(generations), which=TRUE]
    cols <- names(dat.tab)
    for (j in 1:(dim(tmp.probs)[2])){
      #dat.tab[data.table::data.table(c(generations)), paste("compD", j, sep="_") := tmp.probs[,j]]
      data.table::set(dat.tab, rows, cols[6+j], tmp.probs[,j])
      #dat.tab[data.table::data.table(c(generations)), paste("compE", j, sep="_") := phi.mat[,j]]
      data.table::set(dat.tab, rows, cols[36+j], phi.mat[,j])
    }
    dat.tab[data.table::data.table(c(generations)), "comp" := tmp.comp]
  }else{
    ...
    for (j in 1:(dim(tmp.probs)[2])){
      ...
      data.table::set(dat.tab, rows, cols[6+j], tmp.probs[,j])
      #dat.tab[data.table::data.table(c(generations)), paste("compE", j, sep="_") := phi.mat[,j]]
      data.table::set(dat.tab, rows, cols[9+j], phi.mat[,j])
    }
    dat.tab[data.table::data.table(c(generations)), "comp" := tmp.comp]
  }
  return(dat.tab)
