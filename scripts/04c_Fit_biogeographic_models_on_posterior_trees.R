##### Script 04: Fit models in BioGeoBEARS on posterior trees #####
...
# Fit all models on all posterior grafted tree
...
# Comparison of data likelihood according to several models (only possible if using similar biogeographic coding scheme)
  # Density curves to compare model AICc across all posterior trees
...
# utils::install.packages(pkgs = "./packages/phyloch_1.5-3.tar.gz", repos = NULL, type = "source")
...
## Reduce labels to short names to match biogeographic table

Ponerinae_all_posteriors_phylogeny_1534t_short_names <- Ponerinae_all_posteriors_phylogeny_1534t

Ponerinae_all_posteriors_phylogeny_1534t_short_names <- list()
for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t))
{
  Phylo_i <- Ponerinae_all_posteriors_phylogeny_1534t[[i]]
  tips_short_names <- Phylo_i$tip.label
  ...
  # Fix the NewGenus_bucki
  tips_short_names[tips_short_names == "NewGenus_bucki"] <- "Neoponera_bucki"
  
  Phylo_i$tip.label <- tips_short_names
  Ponerinae_all_posteriors_phylogeny_1534t_short_names[[i]] <- Phylo_i
}
class(Ponerinae_all_posteriors_phylogeny_1534t) <- "multiPhylo" 

# Save imputed TreePL posterior phylogenies with short names
saveRDS(object = Ponerinae_all_posteriors_phylogeny_1534t_short_names, file = "./outputs/Grafting_missing_taxa/Ponerinae_all_posteriors_phylogeny_1534t_short_names.rds")

View(cbind(Ponerinae_all_posteriors_phylogeny_1534t[[1]]$tip.label, Ponerinae_all_posteriors_phylogeny_1534t_short_names[[1]]$tip.label))
View(cbind(Ponerinae_all_posteriors_phylogeny_1534t[[10]]$tip.label, Ponerinae_all_posteriors_phylogeny_1534t_short_names[[10]]$tip.label))

# Extract root ages
root_age_list <- lapply(Ponerinae_all_posteriors_phylogeny_1534t, FUN = function (x) max(phytools::nodeHeights(x)))
root_ages <- unlist(root_age_list)
summary(root_ages)
BayesTwin::HPD(sample = root_ages, cred_int = 0.95)
...
# Load imputed TreePL posterior phylogeny with short names
Ponerinae_all_posteriors_phylogeny_1534t_short_names <- readRDS(file = "./outputs/Grafting_missing_taxa/Ponerinae_all_posteriors_phylogeny_1534t_short_names.rds")

## Adjust ages of problematic nodes that are too close to time boundaries (within 1e-05 range)
source("./functions/shiftNodeAge.R")

# Node 2515 in posterior tree 146 is too close to 5.33 My
root_age_i <- root_age <- max(phytools::nodeHeights(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[146]]))
root_age_i - nodeheight(tree = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[146]], node = 2515)
Ponerinae_all_posteriors_phylogeny_1534t_short_names[[146]] <- shiftNodeAge(phy = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[146]],
                                                                            node = 2515, shift = 0.0001)

# Node 2813 in posterior tree 297 is too close to 23.03 My
root_age_i <- root_age <- max(phytools::nodeHeights(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[297]]))
root_age_i - nodeheight(tree = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[297]], node = 2813)
Ponerinae_all_posteriors_phylogeny_1534t_short_names[[297]] <- shiftNodeAge(phy = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[297]],
                                                                            node = 2813, shift = 0.0001)

# Node 1694 in posterior tree 823 is too close to 5.33 My
root_age_i <- root_age <- max(phytools::nodeHeights(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[823]]))
root_age_i - nodeheight(tree = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[823]], node = 1694)
Ponerinae_all_posteriors_phylogeny_1534t_short_names[[823]] <- shiftNodeAge(phy = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[823]],
                                                                            node = 1694, shift = 0.0002)

# Node 2101 in posterior tree 962 is too close to 5.33 My
root_age_i <- root_age <- max(phytools::nodeHeights(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[962]]))
root_age_i - nodeheight(tree = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[962]], node = 2101)
Ponerinae_all_posteriors_phylogeny_1534t_short_names[[962]] <- shiftNodeAge(phy = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[962]],
                                                                            node = 2101, shift = 0.0002)

# Export under Newick format and set path
path_to_tree_list <- list()
for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
# for (i in 962)
{
  # Export under newick format
  write.tree(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[i]], file = paste0("./outputs/Grafting_missing_taxa/All_posterior_phylo_1534t/Ponerinae_phylogeny_1534t_posterior_",i,".tree"))
  
  # Set path to Newick tree
  path_to_tree_i <- paste0("./outputs/Grafting_missing_taxa/All_posterior_phylo_1534t/Ponerinae_phylogeny_1534t_posterior_",i,".tree")
  path_to_tree_i <- BioGeoBEARS::np(path_to_tree_i)
  
  # Store path to Newick tree in list
  path_to_tree_list[[i]] <- path_to_tree_i
}
...
table(Taxa_bioregions_binary_table$Current_name %in% Ponerinae_all_posteriors_phylogeny_1534t_short_names[[1]]$tip.label)
table(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[1]]$tip.label %in% Taxa_bioregions_binary_table$Current_name)
...
pdf(file = "./outputs/Grafting_missing_taxa/Ponerinae_phylogeny_1534t_posterior_1_with_ranges.pdf", width = 20, height = 200)

# Set plotting parameters
par(mar = c(0.1,0.1,0.1,0.1), oma = c(0,0,0,0)) # bltr
# Graph presence/absence using plotTree.datamatrix
range_map <- phytools::plotTree.datamatrix(tree = Ponerinae_all_posteriors_phylogeny_1534t_short_names[[1]],
                                           X = binary_df_factors,
                                           fsize = 0.7, yexp = 1.1,
...
## Add time line
# Extract root age
root_age <- max(phytools::nodeHeights(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[1]]))
# Define ticks
...
# Alternative for trees with root younger than 100.5 (no Early Cretaceous stratum)
time_boundaries_young_roots <- c(5.33, 23.03, 33.9, 56.0, 66.0, 100.5)
writeLines(text = as.character(time_boundaries_young_roots), con = "./input_data/BioGeoBEARS_setup/time_boundaries_young_roots.txt")

path_to_time_strata_boundaries_for_young_roots <- "./input_data/BioGeoBEARS_setup/time_boundaries_young_roots.txt"
path_to_time_strata_boundaries_for_young_roots <- BioGeoBEARS::np(path_to_time_strata_boundaries_for_young_roots)
...
### Loop per posterior trees

for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
# for (i in 962:length(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
{
  ...
  # Detect root edge to adjust time boundaries and strata if needed
  Posterior_phylo_i <- Ponerinae_all_posteriors_phylogeny_1534t[[i]]
  root_age_i <- max(phytools::nodeHeights(tree = Posterior_phylo_i)[, 2])
  
  # Case with all 7 time-strata
  if (root_age_i >= 100.5)
  {
    DEC_run_i <- define_BioGeoBEARS_run(
      ...
      trfn = path_to_tree_list[[i]], # To provide path to the input tree file
    ...
    DEC_run_i$lists_of_states_lists_0based <- lists_of_states_lists_0based
    
  } else { # Case with young roots: keep only 6 time-strata
    ...
    DEC_run_i <- define_BioGeoBEARS_run(
      ...
      trfn = path_to_tree_list[[i]], # To provide path to the input tree file
      geogfn = path_to_geo_data, # To provide path to the LagrangePHYLIP file with binary ranges
      timesfn = path_to_time_strata_boundaries_for_young_roots, # To provide path to optional file defining the stratified time frames (typically, geographic epochs)
      distsfn = NA, # To provide path to optional file defining the changes in distances between bioregions across time-strates (+X models)
      dispersal_multipliers_fn = NA, # To provide path to optional file defining changes in dispersal multipliers across time-strates (+W models)
      area_of_areas_fn = NA, # To provide path to optional file defining the area of each bioregion (used to weight local extinction rates)
      # areas_allowed_fn = path_to_areas_allowed, # To provide path to file defining the allowed areas at a given time-frame.
    ...
    # Add manually the list of allowed states (per time-strata). Very useful to avoid overparametrization when authorizing a few carefully selected 'polymorphic' ranges
    # Keep only 6 time-strata
    DEC_run_i$lists_of_states_lists_0based <- lists_of_states_lists_0based[1:6]
  ...
  DEC_run_i <- readfiles_BioGeoBEARS_run(DEC_run_i)
  
  # Divide the tree up by timeperiods/strata
  DEC_run_i <- BioGeoBEARS::section_the_tree(inputs = DEC_run_i,
  ...
  DEC_run_i$BioGeoBEARS_model_object@params_table
  
  # Check that starting parameter values are inside the min/max
  DEC_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DEC_run_i)
  # Check validity of set-up before run
  check_BioGeoBEARS_run(DEC_run_i)
  
  # Save run settings
  saveRDS(object = DEC_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_run_posterior_",i,".rds"))
  ...
  DIVA_run_i <- DEC_run_i 
  
  # Remove subset sympatric speciation
  DIVA_run_i$BioGeoBEARS_model_object@params_table["s","type"] = "fixed"
  DIVA_run_i$BioGeoBEARS_model_object@params_table["s","init"] = 0.0
  DIVA_run_i$BioGeoBEARS_model_object@params_table["s","est"] = 0.0
  
  # Adjust the fixed values of ysv, ys, y, and v to account for the absence of s
  DIVA_run_i$BioGeoBEARS_model_object@params_table["ysv","type"] = "2-j" # Instead of 3-j in DEC
  DIVA_run_i$BioGeoBEARS_model_object@params_table["ys","type"] = "ysv*1/2" # Instead of ysv*2/3 in DEC
  DIVA_run_i$BioGeoBEARS_model_object@params_table["y","type"] = "ysv*1/2" # Instead of ysv*1/3 in DEC
  DIVA_run_i$BioGeoBEARS_model_object@params_table["v","type"] = "ysv*1/2" # Instead of ysv*1/3 in DEC
  
  # Allow widespread vicariance; Narrow vicariance and Wide-range vicariance are set equiprobable
  DIVA_run_i$BioGeoBEARS_model_object@params_table["mx01v","type"] = "fixed"
  DIVA_run_i$BioGeoBEARS_model_object@params_table["mx01v","init"] = 0.5
  DIVA_run_i$BioGeoBEARS_model_object@params_table["mx01v","est"] = 0.5
  
  # Check information provided as input to BioGeoBEARS for our run
  str(DIVA_run_i, max.level = 2)
  
  # Check that starting parameter values are inside the min/max
  DIVA_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DIVA_run_i) # Check that starting parameter values are inside the min/max
  # Check validity of set-up before run
  check_BioGeoBEARS_run(DIVA_run_i)
  
  # Save run settings
  saveRDS(object = DIVA_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_run_posterior_",i,".rds"))
  ...
  DEC_J_run_i <- DEC_run_i
  
  # Update status of jump speciation parameter to be estimated
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["j","type"] <- "free"
  # Set initial value of J for optimization to an arbitrary low non-null value
  j_start <- 0.0001
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  
  # Check validity of set-up before run
  DEC_J_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DEC_J_run_i) # Check that starting parameter values are inside the min/max
  check_BioGeoBEARS_run(DEC_J_run_i)
  
  # Save run settings
  saveRDS(object = DEC_J_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_J_run_posterior_",i,".rds"))
  
  
  ## Update DIVALIKE + J run object with new parameter settings
  
  DIVA_J_run_i <- DIVA_run_i
  
  # Update status of jump speciation parameter to be estimated
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["j","type"] <- "free"
  # Set initial value of J for optimization to an arbitrary low non-null value
  j_start <- 0.0001
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  
  # Check validity of set-up before run
  DIVA_J_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DIVA_J_run_i) # Check that starting parameter values are inside the min/max
  check_BioGeoBEARS_run(DIVA_J_run_i)
  
  # Save run settings
  saveRDS(object = DIVA_J_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_J_run_posterior_",i,".rds"))
  ...
  # 
  # DEC_W_run_i <- DEC_run_i
  # 
  # # Inform of path to dispersal multiplier matrices
  # DEC_W_run_i$dispersal_multipliers_fn <- path_to_dispersal_multiplier_matrices
  ...
  # # DEC_W_run_i$areas_adjacency_fn <- path_to_adjacency_matrices_for_W_models # Useless if providing $lists_of_states_lists_0based
  # # Adjust list of allowed states per time strata
  # DEC_W_run_i$lists_of_states_lists_0based <- lists_of_states_lists_0based_for_W_models
  # 
  # # Load the dispersal multiplier and modified adjacency matrices from the text files into the model object.
  # DEC_W_run_i <- readfiles_BioGeoBEARS_run(DEC_W_run_i)
  # 
  # # Update status of dispersal multiplier exponent (+W) to be estimated
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["w","type"] <- "free"
  # 
  ...
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["w","init"] <- w_start
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["w","est"] <- w_start # MLE will evolved after optimization
  # 
  # # Inspect the parameter table
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table
  # 
  ...
  # DEC_W_run_i$speedup = FALSE
  # 
  # # Check validity of set-up before run
  # DEC_W_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DEC_W_run_i) # Check that starting parameter values are inside the min/max
  # check_BioGeoBEARS_run(DEC_W_run_i)
  # 
  # # Save run settings
  # saveRDS(object = DEC_W_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_W_run_posterior_",i,".rds"))
  # 
  # 
  # ## Update DIVA + W run object with new parameter settings
  # 
  # DIVA_W_run_i <- DIVA_run_i
  # 
  # # Inform of path to dispersal multiplier matrices
  # DIVA_W_run_i$dispersal_multipliers_fn <- path_to_dispersal_multiplier_matrices
  # # Adjust adjacency matrices and list of allowed states so dispersal multipliers acts on both jump-dispersal (j) AND range extension (d)
  # # DIVA_W_run_i$areas_adjacency_fn <- path_to_adjacency_matrices_for_W_models # Useless if providing $lists_of_states_lists_0based
  # # Adjust list of allowed states per time strata
  # DIVA_W_run_i$lists_of_states_lists_0based <- lists_of_states_lists_0based_for_W_models
  # 
  # # Load the dispersal multiplier and modified adjacency matrices from the text files into the model object.
  # DIVA_W_run_i <- readfiles_BioGeoBEARS_run(DIVA_W_run_i)
  # 
  # # Update status of dispersal multiplier exponent (+W) to be estimated
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["w","type"] <- "free"
  # 
  # # Set initial value of W as 1 such as there is no modulation of the diserpsal multipliers
  # w_start <- 1
  # 
  ...
  # 
  # # Inform initial value of W
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["w","init"] <- w_start
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["w","est"] <- w_start # MLE will evolved after optimization
  # 
  # # Inspect the parameter table
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table
  # 
  # #	ML search for DIVA+W model can be hard to converge, 
  # # So it is recommended to try with different starting point,
  # # and with “speedup" = FALSE to increase number of maxiter in optimx search to 250 instead of 50*nb of free parameters = 150
  # DIVA_W_run_i$speedup = FALSE
  # 
  # # Check validity of set-up before run
  # DIVA_W_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DIVA_W_run_i) # Check that starting parameter values are inside the min/max
  # check_BioGeoBEARS_run(DIVA_W_run_i)
  # 
  # # Save run settings
  # saveRDS(object = DIVA_W_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_W_run_posterior_",i,".rds"))
  ...
  # 
  # DEC_JW_run_i <- DEC_W_run_i
  # 
  # # Update status of jump speciation parameter to be estimated
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["j","type"] <- "free"
  # # Set initial value of J for optimization to an arbitrary low non-null value
  # j_start <- 0.0001
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  # 
  # # Check validity of set-up before run
  # DEC_JW_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DEC_JW_run_i) # Check that starting parameter values are inside the min/max
  # check_BioGeoBEARS_run(DEC_JW_run_i)
  # 
  # # Save run settings
  # saveRDS(object = DEC_JW_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_JW_run_posterior_",i,".rds"))
  # 
  # ## Update DIVALIKE + J run object with new parameter settings
  # 
  # DIVA_JW_run_i <- DIVA_W_run_i
  # 
  # # Update status of jump speciation parameter to be estimated
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["j","type"] <- "free"
  # # Set initial value of J for optimization to an arbitrary low non-null value
  # j_start <- 0.0001
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  # 
  # # Check validity of set-up before run
  # DIVA_JW_run_i <- fix_BioGeoBEARS_params_minmax(BioGeoBEARS_run_object = DIVA_JW_run_i) # Check that starting parameter values are inside the min/max
  # check_BioGeoBEARS_run(DIVA_JW_run_i)
  # 
  # # Save run settings
  # saveRDS(object = DIVA_JW_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_JW_run_posterior_",i,".rds"))
  
  # Print progress every 10 phylogenies
  if (i %% 10 == 0)
  {
    cat(paste0(Sys.time(), " - BioGeoBEARS models set for posterior phylogeny n°",i,"/",length(Ponerinae_all_posteriors_phylogeny_1534t_short_names),"\n"))
  }
}
...
for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
# for (i in 816:length(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
{
  
  ### 6.1/ Run DEC model ####
  
  # Load run settings
  DEC_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_run_posterior_",i,".rds"))
  ...
  # DEC_run_i$num_cores_to_use <- 1
  
  # Fit model using ML while recording logs
  log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DEC_run_log_posterior_",i,".txt"), open = "wt")
  sink(file = log_file, type = "output", split = TRUE)
  cat(paste0(Sys.time(), " - Start DEC model run\n"))
  Start_time <- Sys.time()
  DEC_fit_i <- bears_optim_run(DEC_run_i)
  End_time <- Sys.time()
  ...
  # Save model fit
  saveRDS(object = DEC_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_fit_posterior_",i,".rds"))
  ...
  # DEC_pars_table <- compute_parameter_CI_from_Hessian(DEC_fit_i, plot = T, model_name = "DEC") ; DEC_pars_table
  # 
  # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # DEC_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DEC_fit_i$inputs, include_null_range = DEC_fit_i$inputs$include_null_range)
  ...
  # str(DEC_fit_i)
  # DEC_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # dim(DEC_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # DEC_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # DEC_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  # 
  ...
  # Load run settings
  DIVA_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_run_posterior_",i,".rds"))
  ...
  # Fit model using ML while recording logs
  log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DIVA_run_log_posterior_",i,".txt"), open = "wt")
  sink(file = log_file, type = "output", split = TRUE)
  cat(paste0(Sys.time(), " - Start DIVALIKE model run\n"))
  Start_time <- Sys.time()
  DIVA_fit_i <- bears_optim_run(DIVA_run_i)
  End_time <- Sys.time()
  ...
  # Save model fit
  saveRDS(object = DIVA_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_fit_posterior_",i,".rds"))
  ...
  # # convcode = 1 => Hit maxit (50 x nb of free parameters) without convergence (Should adjust maxit as )
  # 
  # DIVA_fit_i$optim_result
  # # Model as only 2 free parameters = d (range extension) and e (range contraction)
  # # p1 = d = dispersal rate = rate of range expansion between areas along branches = transition from a range with N-areas to a range with N+1 areas
  # # p2 = e = extinction rate within each area along branches = transition from a range with N-areas to a range with N-1 areas
  # 
  # # Inspect uncertainty in parameter estimates
  # DIVA_pars_table <- compute_parameter_CI_from_Hessian(DIVA_fit_i, plot = T, model_name = "DIVALIKE") ; DIVA_pars_table
  # 
  # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # DIVA_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DIVA_fit_i$inputs, include_null_range = DIVA_fit_i$inputs$include_null_range)
  ...
  # 
  # # Inspect marginal likelihoods of ancestral states
  # DIVA_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DIVA model with the MLE parameters
  # dim(DIVA_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # DIVA_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DIVA model with the MLE parameters estimated globally
  # DIVA_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  # 
  
  ### 6.3/ Run DEC+J model ####
  
  ## Load run settings
  DEC_J_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_J_run_posterior_",i,".rds"))
  
  ## Use MLE of DEC model as starting values
  DEC_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_fit_posterior_",i,".rds"))
  
  ## Set starting values for optimization based on MLE in the DEC model
  d_start <- DEC_fit_i$outputs@params_table["d","est"]
  e_start <- DEC_fit_i$outputs@params_table["e","est"]
  
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  DEC_J_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  
  # Save settings before run
  saveRDS(object = DEC_J_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_J_run_posterior_",i,".rds"))
  
  # No parallelization as it seems to be useful only when the number of states is big
  # DEC_J_run_i$num_cores_to_use <- 1
  
  # Fit model using ML while recording logs
  log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DEC_J_run_log_posterior_",i,".txt"), open = "wt")
  sink(file = log_file, type = "output", split = TRUE)
  cat(paste0(Sys.time(), " - Start DEC+J model run\n"))
  Start_time <- Sys.time()
  DEC_J_fit_i <- bears_optim_run(DEC_J_run_i)
  End_time <- Sys.time()
  ...
  # Save model fit
  saveRDS(object = DEC_J_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_J_fit_posterior_",i,".rds"))
  ...
  # 
  # # Inspect uncertainty in parameter estimates
  # DEC_J_pars_table <- compute_parameter_CI_from_Hessian(BioGeoBEARS_output = DEC_J_fit_i, subset_parameter_indices = c(1,3), plot = T, model_name = "DEC+J") ; DEC_J_pars_table
  # 
  # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # DEC_J_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DEC_J_fit_i$inputs, include_null_range = DEC_J_fit_i$inputs$include_null_range)
  ...
  # 
  # # Inspect marginal likelihoods of ancestral states
  # DEC_J_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # dim(DEC_J_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # DEC_J_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # DEC_J_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  
  ### 6.4/ Run DIVALIKE+J model ####
  
  ## Load run settings
  DIVA_J_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_J_run_posterior_",i,".rds"))
  
  ## Use MLE of DIVA model as starting values
  DIVA_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_fit_posterior_",i,".rds"))
  
  ## Set starting values for optimization based on MLE in the DEC model
  d_start <- DIVA_fit_i$outputs@params_table["d","est"]
  e_start <- DIVA_fit_i$outputs@params_table["e","est"]
  
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  DIVA_J_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  
  # Save settings before run
  saveRDS(object = DIVA_J_run_i, file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_J_run_posterior_",i,".rds"))
  
  # No parallelization as it seems to be useful only when the number of states is big
  # DIVA_J_run_i$num_cores_to_use <- 1
  
  # Fit model using ML while recording logs
  log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DIVA_J_run_log_posterior_",i,".txt"), open = "wt")
  sink(file = log_file, type = "output", split = TRUE)
  cat(paste0(Sys.time(), " - Start DIVALIKE+J model run\n"))
  Start_time <- Sys.time()
  DIVA_J_fit_i <- bears_optim_run(DIVA_J_run_i)
  End_time <- Sys.time()
  ...
  # Save model fit
  saveRDS(object = DIVA_J_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_J_fit_posterior_",i,".rds"))
  ...
  # 
  # # Inspect uncertainty in parameter estimates
  # DIVA_J_pars_table <- compute_parameter_CI_from_Hessian(DIVA_J_fit_i, subset_parameter_indices = c(1,3), plot = T, model_name = "DIVALIKE+J") ; DIVA_J_pars_table
  # 
  # 
  # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # DIVA_J_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DIVA_J_fit_i$inputs, include_null_range = DIVA_J_fit_i$inputs$include_null_range)
  # DIVA_J_Qmat <- DIVA_J_mat$Qmat
  # colnames(DIVA_J_Qmat) <- row.names(DIVA_J_Qmat) <- DIVA_J_mat$ranges_list
  # DIVA_J_Qmat
  # 
  # # Inspect final parameter table
  # DIVA_J_fit_i$outputs
  # 
  # # Allows narrow sympatric speciation (y), and narrow AND WIDE vicariance (v)
  # # No subset sympatric speciation (s = 0)
  # # With jump dispersal (j)
  ...
  # 
  # # Inspect marginal likelihoods of ancestral states
  # DIVA_J_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # dim(DIVA_J_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # DIVA_J_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # DIVA_J_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  
  # ### 6.5/ Run DEC+W model ####
  # 
  # ## Load run settings
  # DEC_W_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_W_run_posterior_",i,".rds"))
  # 
  # ## Use MLE of DEC model as starting values
  # DEC_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_fit_posterior_",i,".rds"))
  # 
  # ## Set starting values for optimization based on MLE in the DEC model
  # d_start <- DEC_fit_i$outputs@params_table["d","est"]
  # e_start <- max(DEC_fit_i$outputs@params_table["e","est"], 0.01)
  # 
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  # DEC_W_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  # 
  ...
  # 
  # # No parallelization as it seems to be useful only when the number of states is big
  # # DEC_W_run_i$num_cores_to_use <- 1
  # 
  # # Fit model using ML while recording logs
  # log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DEC_W_run_log_posterior_",i,".txt"), open = "wt")
  # sink(file = log_file, type = "output", split = TRUE)
  # cat(paste0(Sys.time(), " - Start DEC+W model run\n"))
  # Start_time <- Sys.time()
  # DEC_W_fit_i <- bears_optim_run(DEC_W_run_i)
  # End_time <- Sys.time()
  ...
  # 
  # # Save model fit
  # saveRDS(object = DEC_W_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_W_fit_posterior_",i,".rds"))
  # 
  # # ## Inspect results
  # # 
  # # ?optimx
  ...
  # # 
  # # # Inspect uncertainty in parameter estimates
  # # DEC_W_pars_table <- compute_parameter_CI_from_Hessian(DEC_W_fit_i, plot = T, model_name = "DEC+W") ; DEC_W_pars_table
  # # 
  # # 
  # # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # # DEC_W_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DEC_W_fit_i$inputs, include_null_range = DEC_W_fit_i$inputs$include_null_range)
  ...
  # # 
  # # # Inspect marginal likelihoods of ancestral states
  # # DEC_W_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # # dim(DEC_W_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # # DEC_W_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # # DEC_W_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  
  # ### 6.6/ Run DIVALIKE+W model ####
  # 
  # ## Load run settings
  # DIVA_W_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_W_run_posterior_",i,".rds"))
  # 
  # ## Use MLE of DIVA model as starting values
  # DIVA_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_fit_posterior_",i,".rds"))
  # 
  # ## Set starting values for optimization based on MLE in the DEC model
  # d_start <- DIVA_fit_i$outputs@params_table["d","est"]
  # e_start <- max(DIVA_fit_i$outputs@params_table["e","est"], 0.01)
  # 
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  # DIVA_W_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  # 
  # ### Check what happen if using force_sparse == TRUE to save time in computation
  # 
  # # Save settings before run
  ...
  # 
  # # No parallelization as it seems to be useful only when the number of states is big
  # # DIVA_W_run_i$num_cores_to_use <- 1
  # 
  # # Fit model using ML while recording logs
  # log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DIVA_W_run_log_posterior_",i,".txt"), open = "wt")
  # sink(file = log_file, type = "output", split = TRUE)
  # cat(paste0(Sys.time(), " - Start DIVALIKE+W model run\n"))
  # Start_time <- Sys.time()
  # DIVA_W_fit_i <- bears_optim_run(DIVA_W_run_i)
  # End_time <- Sys.time()
  ...
  # 
  # # Save model fit
  # saveRDS(object = DIVA_W_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_W_fit_posterior_",i,".rds"))
  # 
  # # ## Inspect results
  # # 
  # # ?optimx
  ...
  # # 
  # # # Inspect uncertainty in parameter estimates
  # # DIVA_W_pars_table <- compute_parameter_CI_from_Hessian(DIVA_W_fit_i, plot = T, model_name = "DIVALIKE+W") ; DIVA_W_pars_table
  # # 
  # # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # # DIVA_W_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DIVA_W_fit_i$inputs, include_null_range = DIVA_W_fit_i$inputs$include_null_range)
  # # DIVA_W_Qmat <- DIVA_W_mat$Qmat
  # # colnames(DIVA_W_Qmat) <- row.names(DIVA_W_Qmat) <- DIVA_W_mat$ranges_list
  # # DIVA_W_Qmat
  # # 
  # # # Inspect final parameter table
  # # DIVA_W_fit_i$outputs
  # # 
  # # # Allows narrow sympatric speciation (y), and narrow AND WIDE vicariance (v)
  # # # No subset sympatric speciation (s = 0)
  # # # No jump dispersal (j = 0)
  ...
  # # 
  # # # Inspect marginal likelihoods of ancestral states
  # # DIVA_W_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # # dim(DIVA_W_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # # DIVA_W_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # # DIVA_W_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  
  # ### 6.7/ Run DEC+J+W model ####
  # 
  # ## Load run settings
  # DEC_JW_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DEC_JW_run_posterior_",i,".rds"))
  # 
  # ## Use MLE of DEC+J and DEC+W models as starting values
  # DEC_J_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_J_fit_posterior_",i,".rds"))
  # DEC_W_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_W_fit_posterior_",i,".rds"))
  # 
  # ## Set starting values for optimization based on MLE in the DEC+J model
  # d_start <- DEC_J_fit_i$outputs@params_table["d","est"]
  # e_start <- max(DEC_J_fit_i$outputs@params_table["e","est"], 0.01)
  # j_start <- max(DEC_J_fit_i$outputs@params_table["j","est"], 0.0001)
  # w_start <- max(DEC_W_fit_i$outputs@params_table["w","est"], 0.0001)
  # 
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["w","init"] <- w_start
  # DEC_JW_run_i$BioGeoBEARS_model_object@params_table["w","est"] <- w_start # MLE will evolved after optimization
  # 
  # ### Check what happen if using force_sparse == TRUE to save time in computation
  # 
  # # Save settings before run
  ...
  # 
  # # No parallelization as it seems to be useful only when the number of states is big
  # # DEC_JW_run_i$num_cores_to_use <- 1
  # 
  # # Fit model using ML while recording logs
  # log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DEC_JW_run_log_posterior_",i,".txt"), open = "wt")
  # sink(file = log_file, type = "output", split = TRUE)
  # cat(paste0(Sys.time(), " - Start DEC+J+W model run\n"))
  # Start_time <- Sys.time()
  # DEC_JW_fit_i <- bears_optim_run(DEC_JW_run_i)
  # End_time <- Sys.time()
  ...
  # 
  # # Save model fit
  # saveRDS(object = DEC_JW_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_JW_fit_posterior_",i,".rds"))
  # 
  # 
  # # ## Inspect results
  # # 
  ...
  # # 
  # # # Inspect uncertainty in parameter estimates
  # # DEC_JW_pars_table <- compute_parameter_CI_from_Hessian(DEC_JW_fit_i, plot = T, model_name = "DEC+J+W") ; DEC_JW_pars_table
  # # 
  # # 
  # # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # # DEC_JW_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DEC_JW_fit_i$inputs, include_null_range = DEC_JW_fit_i$inputs$include_null_range)
  # # DEC_JW_Qmat <- DEC_JW_mat$Qmat
  # # colnames(DEC_JW_Qmat) <- row.names(DEC_JW_Qmat) <- DEC_JW_mat$ranges_list
  # # DEC_JW_Qmat
  # # 
  # # # Inspect final parameter table
  # # DEC_JW_fit_i$outputs
  # # 
  # # # Allows narrow sympatric speciation (y), narrow vicariance (v), and subset sympatric speciation (s)
  # # # With jump dispersal (j)
  # # # Here, relative weights of y,v,s are fixed to 1 - j/3.
  ...
  # # 
  # # # Inspect marginal likelihoods of ancestral states
  # # DEC_JW_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # # dim(DEC_JW_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # # DEC_JW_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # # DEC_JW_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  
  # ### 6.8/ Run DIVALIKE+J+W model ####
  # 
  # ## Load run settings
  # DIVA_JW_run_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_runs/All_posterior_phylo_1534t/DIVA_JW_run_posterior_",i,".rds"))
  # 
  # ## Use MLE of DIVALIKE+J and DIVALIKE+W models as starting values
  # DIVA_J_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_J_fit_posterior_",i,".rds"))
  # DIVA_W_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_W_fit_posterior_",i,".rds"))
  # 
  # ## Set starting values for optimization based on MLE in the DEC+J model
  # d_start <- DIVA_J_fit_i$outputs@params_table["d","est"]
  # e_start <- max(DIVA_J_fit_i$outputs@params_table["e","est"], 0.01)
  # j_start <- max(DIVA_J_fit_i$outputs@params_table["j","est"], 0.0001)
  # w_start <- max(DIVA_W_fit_i$outputs@params_table["w","est"], 0.0001)
  # 
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["d","init"] <- d_start
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["d","est"] <- d_start # MLE will evolved after optimization
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["e","init"] <- e_start
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["e","est"] <- e_start # MLE will evolved after optimization
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["j","init"] <- j_start
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["j","est"] <- j_start # MLE will evolved after optimization
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["w","init"] <- w_start
  # DIVA_JW_run_i$BioGeoBEARS_model_object@params_table["w","est"] <- w_start # MLE will evolved after optimization
  # 
  # ### Check what happen if using force_sparse == TRUE to save time in computation
  # 
  # # Save settings before run
  # saveRDS(object = DIVA_JW_fit_i, file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_JW_fit_posterior_",i,".rds"))
  # 
  # # No parallelization as it seems to be useful only when the number of states is big
  # # DIVA_JW_run_i$num_cores_to_use <- 1
  # 
  # # Fit model using ML while recording logs
  # log_file <- file(paste0("./outputs/BioGeoBEARS_models/model_logs/All_posterior_phylo_1534t/DIVA_JW_run_log_posterior_",i,".txt"), open = "wt")
  # sink(file = log_file, type = "output", split = TRUE)
  # cat(paste0(Sys.time(), " - Start DIVALIKE+J+W model run\n"))
  # Start_time <- Sys.time()
  # DIVA_JW_fit_i <- bears_optim_run(DIVA_JW_run_i)
  # End_time <- Sys.time()
  ...
  # # 
  # # # Extract Q matrix (only anagenetic transitions. Not affected by multipliers and distances...)
  # # DIVA_JW_mat <- get_Qmat_COOmat_from_BioGeoBEARS_run_object(BioGeoBEARS_run_object = DIVA_JW_fit_i$inputs, include_null_range = DIVA_JW_fit_i$inputs$include_null_range)
  # # DIVA_JW_Qmat <- DIVA_JW_mat$Qmat
  # # colnames(DIVA_JW_Qmat) <- row.names(DIVA_JW_Qmat) <- DIVA_JW_mat$ranges_list
  # # DIVA_JW_Qmat
  # # 
  # # # Inspect final parameter table
  # # DIVA_JW_fit_i$outputs
  # # 
  # # # Allows narrow sympatric speciation (y), and narrow AND WIDE vicariance (v)
  # # # No subset sympatric speciation (s = 0)
  # # # With jump dispersal (j)
  ...
  # # 
  # # # Inspect marginal likelihoods of ancestral states
  # # DIVA_JW_fit_i$total_loglikelihood # log-likelihood of the geographic range data given the phylogenetic tree and DEC model with the MLE parameters
  # # dim(DEC_JW_fit$ML_marginal_prob_each_state_at_branch_top_AT_node) # Likelihood df: Rows = tips + nodes, Columns = states/ranges
  # # DIVA_JW_fit_i$ML_marginal_prob_each_state_at_branch_top_AT_node # Scaled marginal likelihood (probabilities) at each node/tip given the phylogenetic tree and DEC model with the MLE parameters estimated globally
  # # DIVA_JW_fit_i$ML_marginal_prob_each_state_at_branch_bottom_below_node # Same but for at the 'corner' of internal node, meaning at the start of each descendant branch, after cladogenesis/speciation (and eventually cladogenetic transition).
  
  # Print progress every 1 phylogeny
  if (i %% 1 == 0)
  {
    cat(paste0(Sys.time(), " - BioGeoBEARS models fit for posterior phylogeny n°",i,"/",length(Ponerinae_all_posteriors_phylogeny_1534t_short_names),"\n"))
  }
...
for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
{
  ...
  pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DEC_ARE_map_posterior_",i,".pdf"),
      width = 40, height = 150)
  ...
  plot_BioGeoBEARS_results(results_object = DEC_fit_i,
                           analysis_titletxt = paste0("Posterior Tree n°",i,"\nDEC - 2p model"),
  ...
  pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DIVA_ARE_map_posterior_",i,".pdf"),
      width = 40, height = 150)
  ...
  # Plot legend and tree
  plot_BioGeoBEARS_results(results_object = DIVA_fit_i,
                           analysis_titletxt = paste0("Posterior Tree n°",i,"\nDIVALIKE - 2p model"),
                           plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
                           plotsplits = FALSE, # To not plot corner/split states
                           plotlegend = TRUE, tipcex = 0.7, 
  ...
  pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DEC_J_ARE_map_posterior_",i,".pdf"),
      width = 40, height = 150)
  ...
  # Plot legend and tree
  plot_BioGeoBEARS_results(results_object = DEC_J_fit_i,
                           analysis_titletxt = paste0("Posterior Tree n°",i,"\nDEC+J - 3p model"),
                           plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
                           plotsplits = FALSE, # To not plot corner/split states
                           plotlegend = TRUE, tipcex = 0.7, 
  ...
  pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DIVA_J_ARE_map_posterior_",i,".pdf"),
      width = 40, height = 150)
  ...
  # Plot legend and tree
  plot_BioGeoBEARS_results(results_object = DIVA_J_fit_i,
                           analysis_titletxt = paste0("Posterior Tree n°",i,"\nDIVALIKE+J - 3p model"),
                           plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
                           plotsplits = FALSE, # To not plot corner/split states
                           plotlegend = TRUE, tipcex = 0.7, 
  ...
  # DEC_W_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_W_fit_posterior_",i,".rds"))
  # 
  # pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DEC_W_ARE_map_posterior_",i,".pdf"),
  #     width = 40, height = 150)
  # 
  # # Subdivide our plotting area using layout
  # layout(matrix(1:2, 1, 2), widths = c(0.2, 0.8))
  ...
  # 
  # # Plot legend and tree
  # plot_BioGeoBEARS_results(results_object = DEC_W_fit_i,
  #                          analysis_titletxt = paste0("Posterior Tree n°",i,"\nDEC+W - 3p model"),
  #                          plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
  #                          plotsplits = FALSE, # To not plot corner/split states
  #                          plotlegend = TRUE, tipcex = 0.7, 
  ...
  # DIVA_W_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_W_fit_posterior_",i,".rds"))
  # 
  # pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DIVA_W_ARE_map_posterior_",i,".pdf"),
  #     width = 40, height = 150)
  # 
  # # Subdivide our plotting area using layout
  # layout(matrix(1:2, 1, 2), widths = c(0.2, 0.8))
  ...
  # 
  # # Plot legend and tree
  # plot_BioGeoBEARS_results(results_object = DIVA_W_fit_i,
  #                          analysis_titletxt = paste0("Posterior Tree n°",i,"\nDIVALIKE+W - 3p model"),
  #                          plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
  #                          plotsplits = FALSE, # To not plot corner/split states
  #                          plotlegend = TRUE, tipcex = 0.7, 
  ...
  # DEC_JW_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DEC_JW_fit_posterior_",i,".rds"))
  # 
  # pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DEC_JW_ARE_map_posterior_",i,".pdf"),
  #     width = 40, height = 150)
  # 
  # # Subdivide our plotting area using layout
  # layout(matrix(1:2, 1, 2), widths = c(0.2, 0.8))
  ...
  # 
  # # Plot legend and tree
  # plot_BioGeoBEARS_results(results_object = DEC_JW_fit_i,
  #                          analysis_titletxt = paste0("Posterior Tree n°",i,"\nDEC+J+W - 4p model"),
  #                          plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
  #                          plotsplits = FALSE, # To not plot corner/split states
  #                          plotlegend = TRUE, tipcex = 0.7, 
  ...
  # DIVA_JW_fit_i <- readRDS(file = paste0("./outputs/BioGeoBEARS_models/model_fits/All_posterior_phylo_1534t/DIVA_JW_fit_posterior_",i,".rds"))
  # 
  # pdf(file = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/DIVA_JW_ARE_map_posterior_",i,".pdf"),
  #     width = 40, height = 150)
  # 
  # # Subdivide our plotting area using layout
  # layout(matrix(1:2, 1, 2), widths = c(0.2, 0.8))
  ...
  # 
  # # Plot legend and tree
  # plot_BioGeoBEARS_results(results_object = DIVA_JW_fit_i,
  #                          analysis_titletxt = paste0("Posterior Tree n°",i,"\nDIVALIKE+J+W - 4p model"),
  #                          plotwhat = "pie", # If setting "test", will only plot the states with the highest marginal likelihood
  #                          plotsplits = FALSE, # To not plot corner/split states
  #                          plotlegend = TRUE, tipcex = 0.7, 
  #                          titlecex = 3.0,
  ...
  all_ARE_maps_path <- list.files(path = "./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/", pattern = paste0("ARE_map_posterior_",i,".pdf"), full.names = T)
  
  qpdf::pdf_combine(input = all_ARE_maps_path, output = paste0("./outputs/Ancestral_range_estimates_maps/All_posterior_phylo_1534t/All_models_ARE_maps_posterior_",i,".pdf"))
  
  ## Print progress every 10 phylogenies
  if (i %% 10 == 0)
  {
    cat(paste0(Sys.time(), " - ARE plotted for posterior phylogeny n°",i,"/",length(Ponerinae_all_posteriors_phylogeny_1534t_short_names),"\n"))
  }
}
...
### 8.2/ Build summary tables for all models x all posterior trees ####

### Loop per posterior trees

BIOGeoBEARS_models_comparison_all_posteriors <- data.frame() # Initiate summary df for all posterior phylogenies
for (i in seq_along(Ponerinae_all_posteriors_phylogeny_1534t_short_names))
# for (i in 1:95)
{
  # Extract number of observations (terminal taxa)
  nb_obs <- length(Ponerinae_all_posteriors_phylogeny_1534t_short_names[[i]]$tip.label)
  ...
  ## Combine models' fit in a list
  models_list_i <- list(DEC_fit_i, DIVA_fit_i, DEC_J_fit_i, DIVA_J_fit_i)
  # models_list_i <- list(DEC_fit_i, DIVA_fit_i, DEC_J_fit_i, DIVA_J_fit_i, DEC_W_fit_i, DIVA_W_fit_i, DEC_JW_fit_i, DIVA_JW_fit_i)
  # models_list_i <- list(DEC_fit_i, DIVA_fit_i, DEC_J_fit_i, DIVA_J_fit_i, DEC_W_fit_i, DIVA_W_fit_i)
  
  ## Extract ln-likelihood, number of parameters, and AIC from models' list
  BIOGeoBEARS_models_comparison_i <- data.frame(Posterior_phylo_ID = i,
                                                model = c("DEC", "DIVALIKE", "DEC+J", "DIVALIKE+J"),
                                                logLk = sapply(X = models_list_i, FUN = get_LnL_from_BioGeoBEARS_results_object),
                                                k = sapply(X = models_list_i, FUN = extract_k_from_BioGeoBEARS_output))
  
  # BIOGeoBEARS_models_comparison_i <- data.frame(Posterior_phylo_ID = i,
  #                                               model = c("DEC", "DIVALIKE", "DEC+J", "DIVALIKE+J", "DEC+W", "DIVALIKE+W", "DEC+J+W", "DIVALIKE+J+W"),
  #                                               logLk = sapply(X = models_list_i, FUN = get_LnL_from_BioGeoBEARS_results_object),
  #                                               k = sapply(X = models_list_i, FUN = extract_k_from_BioGeoBEARS_output))
  # BIOGeoBEARS_models_comparison_i <- data.frame(Posterior_phylo_ID = i,
  #                                               model = c("DEC", "DIVALIKE", "DEC+J", "DIVALIKE+J", "DEC+W", "DIVALIKE+W"),
  #                                               logLk = sapply(X = models_list_i, FUN = get_LnL_from_BioGeoBEARS_results_object),
  #                                               k = sapply(X = models_list_i, FUN = extract_k_from_BioGeoBEARS_output))
  
  # Compute AIC from logLk and k
  BIOGeoBEARS_models_comparison_i$AIC <- unlist(purrr::map2(.x = BIOGeoBEARS_models_comparison_i$logLk, .y = BIOGeoBEARS_models_comparison_i$k, .f = compute_AIC))
  
  # Compute AICc from AIC, k and nobs
  BIOGeoBEARS_models_comparison_i$AICc <- unlist(purrr::map2(.x = BIOGeoBEARS_models_comparison_i$AIC, nobs = nb_obs, .y = BIOGeoBEARS_models_comparison_i$k, .f = compute_AICc))
  
  # Compute Delta best AICc
  best_AICc <- min(BIOGeoBEARS_models_comparison_i$AICc)
  BIOGeoBEARS_models_comparison_i$delta_best_AICc <- BIOGeoBEARS_models_comparison_i$AICc - best_AICc
  
  # Compute Delta DEC+J AICc
  BIOGeoBEARS_models_comparison_i$delta_DECJ_AICc <- BIOGeoBEARS_models_comparison_i$AICc - BIOGeoBEARS_models_comparison_i$AICc[which(BIOGeoBEARS_models_comparison_i$model == "DEC+J")]
  
  # Compute Akaike's weights from AIC
  BIOGeoBEARS_models_comparison_i$Akaike_weights <- round(phytools::aic.w(BIOGeoBEARS_models_comparison_i$AICc), 3) * 100
  
  # Extract MLE parameters
  models_MLE_pars_i <- t(sapply(X = models_list_i, FUN = extract_MLE_params_with_XW_from_BioGeoBEARS_output))
  
  # Merge with summary table
  BIOGeoBEARS_models_comparison_i <- cbind(BIOGeoBEARS_models_comparison_i, models_MLE_pars_i)
  
  # Display results
  BIOGeoBEARS_models_comparison_i
  
  ## Save results
  saveRDS(object = BIOGeoBEARS_models_comparison_i, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/All_posterior_phylo_1534t/BIOGeoBEARS_models_comparison_posterior_",i,".rds"))
  # Export results
  write.xlsx(x = BIOGeoBEARS_models_comparison_i, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/All_posterior_phylo_1534t/BIOGeoBEARS_models_comparison_posterior_",i,".xlsx"))
  
  ## Store in unique summary data.frame
  BIOGeoBEARS_models_comparison_all_posteriors <- rbind(BIOGeoBEARS_models_comparison_all_posteriors, BIOGeoBEARS_models_comparison_i)
  
  ## Print progress every 10 phylogenies
  if (i %% 10 == 0)
  {
    # Save summary df for all posterior phylogenies
    saveRDS(object = BIOGeoBEARS_models_comparison_all_posteriors, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/BIOGeoBEARS_models_comparison_all_posteriors.rds"))
    # Export results
    write.xlsx(x = BIOGeoBEARS_models_comparison_all_posteriors, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/BIOGeoBEARS_models_comparison_all_posteriors.xlsx"))
    
    cat(paste0(Sys.time(), " - Model evaluations extracted for posterior phylogeny n°",i,"/",length(Ponerinae_all_posteriors_phylogeny_1534t_short_names),"\n"))
  }
}
...
##### 9/ Compare models AICc across all posterior trees ####

## Goal = Robustness analysis to show that grafting and dating uncertainty do not affect the selection of the best model

### 9.1/ Compute summary stats across all posterior trees ####

BIOGeoBEARS_models_comparison_all_posteriors_aggregated_summary <- BIOGeoBEARS_models_comparison_all_posteriors %>% 
  group_by(model) %>% 
  dplyr::summarize(k = median(k),
                   # For delta DEC+J AICc
                   mean_delta_DECJ_AICc = mean(delta_DECJ_AICc),
                   median_delta_DECJ_AICc = median(delta_DECJ_AICc),
                   HPD_0.025_delta_DECJ_AICc = BayesTwin::HPD(delta_DECJ_AICc, cred_int = 0.95)[1],
                   HPD_0.975_delta_DECJ_AICc = BayesTwin::HPD(delta_DECJ_AICc, cred_int = 0.95)[2],
                   # For Aikake's weights
                   mean_Akaike = mean(Akaike_weights),
                   median_Akaike = median(Akaike_weights),
                   HPD_0.025_Akaike = BayesTwin::HPD(Akaike_weights, cred_int = 0.95)[1],
                   HPD_0.975_Akaike = BayesTwin::HPD(Akaike_weights, cred_int = 0.95)[2],
                   # For parameters
                   mean_d = mean(d),
                   median_d = median(d),
                   HPD_0.025_d = BayesTwin::HPD(d, cred_int = 0.95)[1],
                   HPD_0.975_d = BayesTwin::HPD(d, cred_int = 0.95)[2],
                   mean_e = mean(e),
                   median_e = median(e),
                   HPD_0.025_e = BayesTwin::HPD(e, cred_int = 0.95)[1],
                   HPD_0.975_e = BayesTwin::HPD(e, cred_int = 0.95)[2],
                   mean_y = mean(y),
                   median_y = median(y),
                   HPD_0.025_y = BayesTwin::HPD(y, cred_int = 0.95)[1],
                   HPD_0.975_y = BayesTwin::HPD(y, cred_int = 0.95)[2],
                   mean_v = mean(v),
                   median_v = median(v),
                   HPD_0.025_v = BayesTwin::HPD(v, cred_int = 0.95)[1],
                   HPD_0.975_v = BayesTwin::HPD(v, cred_int = 0.95)[2],
                   mean_s = mean(s),
                   median_s = median(s),
                   HPD_0.025_s = BayesTwin::HPD(s, cred_int = 0.95)[1],
                   HPD_0.975_s = BayesTwin::HPD(s, cred_int = 0.95)[2],
                   mean_j = mean(j),
                   median_j = median(j),
                   HPD_0.025_j = BayesTwin::HPD(j, cred_int = 0.95)[1],
                   HPD_0.975_j = BayesTwin::HPD(j, cred_int = 0.95)[2])

# Save summary df aggregating results across all posterior trees
saveRDS(object = BIOGeoBEARS_models_comparison_all_posteriors_aggregated_summary, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/BIOGeoBEARS_models_comparison_all_posteriors_aggregated_summary.rds"))
# Export results
write.xlsx(x = BIOGeoBEARS_models_comparison_all_posteriors_aggregated_summary, file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/BIOGeoBEARS_models_comparison_all_posteriors_aggregated_summary.xlsx"))


### 9.2/ Density curves of Delta AICc per model type across all posterior trees ####

# AICc are not comparable across posterior runs because data are different!
# Use the Delta AICc vs. DEC+J AICc as a comparable metric

pdf(file = paste0("./outputs/BioGeoBEARS_models/model_comparisons/BIOGeoBEARS_models_comparison_Delta_AICc_density_ggplot.pdf"),
    width = 10, height = 6)

BIOGeoBEARS_models_comparison_Delta_AICc_density_ggplot <- BIOGeoBEARS_models_comparison_all_posteriors %>% 
  filter(model != "DEC+J") %>% 
  ggplot() +
  
  # Plot density curves
  geom_density(mapping = aes(x = delta_DECJ_AICc, group = model, fill = model),
               alpha = 0.5) +
  
  # Adjust color scheme and legend
  scale_fill_manual(name = "Models", breaks = c("DIVALIKE", "DIVALIKE+J", "DEC"),
                    labels = c("DIVALIKE", "DIVALIKE + J", "DEC"), values = c("orange", "limegreen", "dodgerblue")) +
  
  # Add h = 0 for DEC+J
  geom_vline(xintercept = 0, linetype = 2, linewidth = 1.5, col = "red") +
  
  # Set plot title +
  ggtitle(label = paste0("Model evaluation across 1000 posterior trees")) +
...
# Plot
print(BIOGeoBEARS_models_comparison_Delta_AICc_density_ggplot)
