...
for (i in 1:nrow(Phylogeny_samples_data))
{
  ...
  focal_Extraction_code <- Phylogeny_samples_data$`Extraction code`[i]
  focal_Specimen_code <- Phylogeny_samples_data$`Specimen code`[i]
...
AntWeb_database_curated <- AntWeb_database_curated[AntWeb_database_curated$Current_status %in% c("valid", "morphotaxon"), ]
...
## 2.3.4/ Curate names so they match between AntWeb, the phylogeny, and this database ####
